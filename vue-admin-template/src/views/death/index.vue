  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="死亡日期" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.death_time">{{ utc2beijing(scope.row.death_time, 2) }}</span>
          <span v-else>--</span>
        </template>
      </el-table-column>
      <el-table-column prop="death_note" align="center" label="备注" />
...
export default {
  ...
  methods: {
    ...
    toDetails(index, row) {
      this.$router.push({
        path: '/epidemic/cure-or-death/detail',
        query: {
          id: row.id,
          type: 'death'
        }
